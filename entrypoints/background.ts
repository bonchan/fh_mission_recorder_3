...
export default defineBackground(() => {
  ...
  browser.runtime.onMessage.addListener(async (message, sender) => {
    ...
    else if (message.type === 'OPEN_FLIGT_PLANNING_DASHBOARD') {
      const { orgId, projectId, sourceTabId, debugMode } = message;
      const url = browser.runtime.getURL(`/flightplanningview.html?orgId=${orgId}&projectId=${projectId}&sourceTabId=${sourceTabId}&debugMode=${debugMode}`);
      ...
      if (jsonPayload.biz_code === 'device_osd') {
        ...
        try {
          ...
          const liveDroneData: LiveDroneData = {
            ...
            yaw: host.attitude_head,
            pitch: payload.gimbal_pitch,
