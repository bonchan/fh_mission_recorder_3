...
export function DashboardView() {
  ...
  const handleExportMission = async (mission: Mission) => {
    if (mission.waypoints.length == 0) {
      showToast('Mission has no waypoints!', '', 'warning')
      return
    }
  ...
  const handleUploadMission = async (mission: Mission) => {
    ...
    if (!mission.orgId || !mission.projectId) {
      showToast('Mission has no orgId or projectId!', '', 'warning')
      return
    }
    ...
    try {
      ...
      const stsResponse = await getStorageUploadCredentials(mission.orgId, mission.projectId); // Assuming this is where it lives
